# sda-test-tim1
## SDA Software Testing Course Resources


### 1. Contact

- Slack - sdatesttim1.slack.com
- [LinkedIn](https://www.linkedin.com/in/rusu-dan/)

### 2. [Postman - API test tool](https://github.com/danrusu/sda-test-tim1/tree/master/postman)

- [Play smart with the backend](https://bit.ly/2LDEjo7) - [repository](https://github.com/danrusu/play-smart-with-the-backend)
 
### 3. [Basic JavaScript](https://github.com/danrusu/sda-test-tim1/tree/master/js/homework)

### 4. Java

- [OOP basics](https://github.com/danrusu/sda-test-tim1/tree/master/java/basics/src/oop)

- [Selenium WebDriver](https://github.com/danrusu/sda-test-tim1/tree/master/java/basics/src/selenium)

### 5. Test Project

   #### 5.1 [Frontend application - target for manual and automated UI testing](http://danrusu.ro/calculate/app.html)
   #### 5.2 [Frontend & backend application - target for UI & API testing](http://danrusu.ro/calculate/appApi.html)

- [Requirements](https://github.com/danrusu/sda-test-tim1/tree/master/test-project/requirements)

- [Tasks](https://github.com/danrusu/sda-test-tim1/blob/master/test-project/tasks.md)

- [downloaded web pages](https://github.com/danrusu/sda-test-tim1/tree/master/test-project/calculate)

## What's next (recommended)

- [java-programming-course](https://testautomationu.applitools.com/java-programming-course/)
- [selenium-webdriver-tutorial-java](https://testautomationu.applitools.com/selenium-webdriver-tutorial-java/)
- [automating-your-api-tests-with-rest-assured](https://testautomationu.applitools.com/automating-your-api-tests-with-rest-assured/)

